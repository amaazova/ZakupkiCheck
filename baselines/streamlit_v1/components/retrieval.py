"""Similar-case retrieval over fas_findings.jsonl."""
...
CHROMA_DIR = PROJECT_ROOT / "workspace" / "streamlit" / "data" / "chroma"
COLLECTION_NAME = "fas_findings_v1"
EMBEDDING_MODEL = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"


def _load_findings() -> list[dict]:
    """Read fas_findings.jsonl into a list of dicts; one entry per FAS decision."""
    out: list[dict] = []
...
def _build_corpus_text(finding: dict) -> str:
    """Concatenate fields used for embedding/indexing one FAS decision."""
    pieces: list[str] = []
...
class _Retriever:
    """Lazy-initialized retriever; chooses ChromaDB or TF-IDF backend."""
    ...
    def query(self, text: str, top_k: int) -> list[dict]:
        ...
        if self._backend == "chroma" and self._collection is not None:
            ...
            for eid, meta, dist in zip(ids, metadatas, distances):
                meta = meta or {}
                # ChromaDB returns squared L2 by default; map to a simple similarity score.
                # Lower distance == more similar. Clamp to [0, 1].
...
def find_similar_cases(text: str, top_k: int = 3) -> list[dict]:
    """Public entry point. Returns up to top_k similar FAS decisions."""
    return _get_retriever().query(text, top_k=top_k)


def retriever_backend() -> str:
    """Backend name for display: 'chroma' | 'tfidf' | 'none'."""
