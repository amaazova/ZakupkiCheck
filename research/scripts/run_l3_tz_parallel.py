"""W2 Session B — L3 TZ few-shot extraction (parallel workers)."""
...
def load_prompts() -> tuple[str, str]:
    ...
    filled = PROMPTS_DIR / "tz_l3_user_prompt_template.filled.md"
    if not filled.is_file():
        raise SystemExit(
            f"{filled} not found — run `python -m workspace.scripts.build_l3_few_shot` first."
        )
    return system, filled.read_text(encoding="utf-8")
...
def main() -> None:
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="run 8 random episodes to tz_features_L3_smoke.jsonl")
    ...
    if args.smoke:
        ...
        output_path = EVAL_DIR / "tz_features_L3_smoke.jsonl"
        ...
        output_path = EVAL_DIR / "tz_features_L3.jsonl"

    log_path = output_path.with_name(output_path.stem + "_log.jsonl")
    completed = load_completed_ids(output_path)
    print(f"[L3-par] workers={args.workers}  output={output_path.name}  "
    ...
    print(f"[L3-par] pending: {len(pending)}/{len(episodes)} episodes")
    if not pending:
        print("[L3-par] nothing to do.")
        return
    ...
    t0 = time.monotonic()
    pbar = tqdm(total=len(pending), desc=f"L3-par[{args.workers}w]", unit="ep")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                print(f"\n[L3-par] worker error for {eid}: {type(e).__name__}: {e}")
            pbar.update(1)
