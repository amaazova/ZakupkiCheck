"""W1 Session B — L1 TZ open extraction with parallel workers."""
...
class SharedStats:
    """Aggregate counters shared across worker threads."""
    ...
    *,
    ...
    try:
        ...
        record = {
            ...
            "model": model,
            "status": "template_missing_key",
            "error": f"missing template key: {e}",
            "ts_utc": _utc_now(),
        }
        with OUTPUT_LOCK:
...
def main() -> None:
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="run 8 random episodes to tz_features_L1_smoke_par.jsonl")
    ...
    if args.smoke:
        ...
        output_path = EVAL_DIR / "tz_features_L1_smoke_par.jsonl"
        ...
        output_path = EVAL_DIR / "tz_features_L1.jsonl"

    log_path = output_path.with_name(output_path.stem + "_log.jsonl")

    completed = load_completed_ids(output_path)
    print(f"[parallel] workers={args.workers}  output={output_path.name}  "
    ...
    print(f"[parallel] pending: {len(pending)}/{len(episodes)} episodes")
    ...
    t0 = time.monotonic()
    pbar = tqdm(total=len(pending), desc=f"L1-par[{args.workers}w]", unit="ep")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                # Surface unexpected errors but keep the pool running
                print(f"\n[parallel] worker error for {eid}: {type(e).__name__}: {e}")
            pbar.update(1)
            pbar.set_postfix(
                cost=f"${stats.cost_usd:.3f}",
                fail=stats.http_failures,
                parse=stats.parse_failures,
            )

    pbar.close()
    dt = time.monotonic() - t0
    print(
        f"\n=== run_l1_tz_parallel summary ===\n"
        ...
        f"  throughput:       {stats.processed/max(dt,1)*60:.2f} ep/min "
        f"({stats.processed/max(dt,1):.3f} ep/s)\n"
