"""W2 Session B — L2 TZ taxonomy-guided extraction (parallel workers)."""
...
TAXONOMY_HINTS_PATH = EVAL_DIR / "taxonomy_hints.json"
TZ_CHAR_LIMIT = 150_000
DEFAULT_HINT = (
    "различные нарушения требований к описанию объекта закупки по ст. 33 44-ФЗ"
)
...
def load_prompts() -> tuple[str, str]:
    ...
    user_tmpl = (PROMPTS_DIR / "tz_l2_user_prompt_template.md").read_text(encoding="utf-8")
    return system, user_tmpl


def load_taxonomy_hints() -> dict[str, str]:
    return json.loads(TAXONOMY_HINTS_PATH.read_text(encoding="utf-8"))


def prepare_episodes(eval_df: pd.DataFrame, hints: dict[str, str]) -> list[dict]:
    ...
    for ep in episodes:
        ...
        cluster_key = str(ep.get("family"))
        ep["taxonomy_hint"] = hints.get(cluster_key, DEFAULT_HINT)
    return episodes
...
class SharedStats:
    __slots__ = ("processed", "skipped", "http_failures", "parse_failures",
                 "retries", "cost_usd", "input_tokens", "output_tokens", "by_status")
    ...
    def merge(self, **kw) -> None:
        with STATS_LOCK:
            for k, v in kw.items():
                if k == "by_status":
                    s = self.by_status.get(v, 0)
                    self.by_status[v] = s + 1
                else:
...
EXTRA_KEYS = ("notice_id", "decision_id", "family", "verdict", "stratum",
              "quality_tier", "taxonomy_hint")
...
def process_episode(
    ...
    *,
    ...
    stats.merge(
        processed=1, cost_usd=result.cost_usd,
...
def main() -> None:
    ...
    if args.smoke:
        ...
        output_path = EVAL_DIR / "tz_features_L2.jsonl"

    log_path = output_path.with_name(output_path.stem + "_log.jsonl")
    completed = load_completed_ids(output_path)
    print(f"[L2-par] workers={args.workers}  output={output_path.name}  "
          f"already_completed={len(completed)}  hints_loaded={len(hints)}")

    episodes = prepare_episodes(df, hints)
    pending = [ep for ep in episodes if ep["episode_id"] not in completed]
    print(f"[L2-par] pending: {len(pending)}/{len(episodes)} episodes")
    if not pending:
        print("[L2-par] nothing to do.")
        return
    ...
    t0 = time.monotonic()
    pbar = tqdm(total=len(pending), desc=f"L2-par[{args.workers}w]", unit="ep")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                print(f"\n[L2-par] worker error for {eid}: {type(e).__name__}: {e}")
            pbar.update(1)
